...
function parseExtraction(value: unknown): ExtractionResult {
  ...
  const places = data.places.map((item): ExtractedPlace => {
    ...
    return { name: p.name.trim(), type: p.type as PlaceType, category: typeof p.category === "string" ? p.category.trim() : undefined, source: typeof p.source === "string" ? p.source : "攻略输入", tips: p.tips.map((tip) => tip.trim()).filter(Boolean).slice(0, 2), estimatedDurationMinutes: duration, durationSource: p.durationSource as DurationSource };
  });
